...
def get_nodelist(entity_list, File_name, ann, eType_list): #create entityList file, new add eType_list
    ...
    f = file('GetEdgeNode/graph_entityList.txt', 'w') # open for 'w'riting
    f1 = file('GetEdgeNode/graph_eTypeList.txt', 'w') #new
...
#    entity_list = get_nodelist(File_name,ann)
    ...
    for index,item in enumerate(sorted_ann):
        ...
        if index == 0:
            ...
            if entity_list.index(item[3]) != entity_list.index(main_entity[-1][3]):
                ...
                modifier_edge.append([entity_list.index(main_entity[-1][3]), entity_list.index(item[3])])
    ...
    f = file('GetEdgeNode/directed_edgeList.txt', 'w') # open for 'w'riting
